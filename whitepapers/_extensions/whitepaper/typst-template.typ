...
#let callout(type: "info", title: none, body: none, ..sink) = {
  let (bg, border, icon) = if type == "info" {
    (info-bg, info-border, "💡")
...
// Shorthand functions
#let info(title: none, body) = callout(type: "info", title: title, body)
#let warning(title: none, body) = callout(type: "warning", title: title, body)
#let success(title: none, body) = callout(type: "success", title: title, body)
#let danger(title: none, body) = callout(type: "danger", title: title, body)
...
#let whitepaper(
  ...
  // List settings - spacing between items + space before list (after colon)
  set list(
    indent: 1em,
    body-indent: 0.5em,
    spacing: 1em,
  ...
  // Add vertical space before lists/enums to visually separate from intro colon
  show list: it => { v(0.4em, weak: true); it }
  show enum: it => { v(0.4em, weak: true); it }
